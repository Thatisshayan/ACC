...
function Stop-PidFile {
  ...
  try {
    ...
    if ($processId -gt 0) {
      ...
      Write-LauncherLog ('Stopped pid {0} from {1}' -f $processId, $Path)
    }
  } catch {
...
function Remove-StaleFile {
  ...
  if (Test-Path $Path) {
    try {
      ...
      Write-LauncherLog ('Removed stale file {0}' -f $Path)
    } catch {
      Write-LauncherLog ('Failed to remove stale file {0}: {1}' -f $Path, $_.Exception.Message)
    }
  }
}

Remove-StaleFile -Path (Join-Path $runDir 'telegram-bot.lock.json')
