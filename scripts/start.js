...
async function main() {
  ...
  // Run auto-migrations on server boot (Task 8)
  console.log('[start] running database migrations...');
  try {
    const migrations = require("./migrate.js");
    await migrations.run();
  } catch (err) {
    console.error('[start] Migrations failed on boot. Exiting:', err.message);
    process.exit(1);
  }
  ...
  if (!process.env.TELEGRAM_BOT_TOKEN) {
    ...
    console.log("[start] Hosted deployment detected — using webhook mode for Telegram bot.");
...
main().catch(err => {
  console.error('[start] Fatal server boot error:', err && err.stack || err);
  process.exit(1);
});
