...
**Confirmed finding to start from (verified 2026-08-01):** `cloud/api/alphonsoBridge.js` defines three routes — `GET /status`, `GET /packets`, `POST /` — with **zero authentication or signature verification of any kind**, and is mounted in `cloud/server.js` at `app.use("/api/alphonso-bridge", alphonsoBridge)` with no middleware wrapper either. Compare this to `cloud/api/telegramWebhook.js`, which is genuinely meant to be hit by an external service (Telegram) and correctly uses a `requireTelegramSecret()` check instead of bearer auth — that's the right pattern for an externally-triggered webhook. `alphonsoBridge.js` has no equivalent. Also note `cloud/services/alphonsoBridgeService.js` (a *different* file) already has a working `timingSafeStringEqual` bearer-token check at line 126 — it's unclear from the route file alone whether that service's auth is meant to protect these routes and just isn't wired in, or whether this route file is a separate, forgotten surface. Investigate and fix.

1. Read `cloud/api/alphonsoBridge.js`, `cloud/services/alphonsoBridgeService.js`, and `docs/ALPHONSO_BRIDGE_RECEIVER.md` (if it describes the intended trust model) to determine: is this meant to be an authenticated internal endpoint, or an externally-triggered webhook like the Telegram one? Decide based on evidence, state your reasoning in the PR log.
2. Apply the correct fix: either wire in the existing bearer-token check from `alphonsoBridgeService.js` (reuse it, don't duplicate it — extract to a shared middleware if needed), or if it's meant to be a webhook, add a comparable secret/signature check following the `requireTelegramSecret()` pattern. Either way, all three routes must require *something* — leaving any of them fully open is not acceptable unless the doc explicitly says otherwise, in which case say so in your report instead of guessing.
3. Write `cloud/api/alphonsoBridge.test.js` proving: unauthenticated requests to all three routes are rejected, correctly authenticated requests succeed.
4. **Systematic sweep, not just this one file:** every `app.use(...)` mount in `cloud/server.js` (there are ~25) either has an auth middleware in the `app.use(...)` call itself, OR the router file self-gates every route internally (like `phoneRoutes.js` and `securityApproval.js` already correctly do), OR is a legitimately public route (health check, static UI assets, public webhook with its own secret check). Go through the full mount list in `cloud/server.js` (roughly lines 264–398) and classify every single one into "properly gated" / "public-by-design, verified" / "gap found." Fix any real gap the same way you fixed `alphonsoBridge.js`. Record your classification table in the PR log even for the ones that were already fine — this is the audit deliverable, not just the fix.
5. Review the CORS config (`app.use(cors({...}))` near line 95) and the rate limiter (`limiter`, near line 122) — confirm they apply broadly enough (check the `skip` logic at line 66) and aren't accidentally excluding routes that should be rate-limited. Fix if you find a real gap; otherwise state you checked and it's fine.
