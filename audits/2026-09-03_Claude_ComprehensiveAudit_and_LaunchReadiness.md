...
7. **IMAP app passwords aren't actually encrypted.**
   `cloud/services/emailMonitor.js:32` does `Buffer.from(password).toString('base64')`
   — trivially reversible, despite a comment promising AES-256 and despite the
   migration's own column comment saying "stored encrypted." A working AES-256-GCM
   helper already exists and is unused here: `cloud/messages/encryption.js`. **Fix**:
   route through that helper (or a dedicated instance of it) before insert. Not
   touched this session — scope was the codeExec RCE/SSRF; this is an independent
...
## 3. P1 — High (none fixed this session, all still open)
...
3. **Two divergent frontend API clients** (`ui/src/lib/api.js` vs `ui/src/api.js`)
   ...
   armed footgun the moment someone wires one up. Delete one client.
4. **TwiML injection in the Twilio call task** (`cloud/api/phoneRoutes.js:54`) —
   `body.message` interpolated unescaped into a `<Say>` element; an operator-role
   caller could inject `<Redirect>` to hijack the call flow.
...
5. Desktop preload exposes `backendStatus()`/`retryBackendStart()` IPC calls with
   no registered handler in `main.js`; also no `will-navigate` guard (narrow risk
   given the tight preload whitelist, but cheap to close).
