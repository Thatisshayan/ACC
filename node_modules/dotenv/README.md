...
```
$ dotenvx set HELLO Production -f .env.REDACTED
$ echo "console.log('Hello ' + process.env.HELLO)" > index.js

$ DOTENV_PRIVATE_KEY_PRODUCTION="<.env.REDACTED private key>" dotenvx run -- node index.js
⟐ injected env (2) from .env.REDACTED · dotenvx@1.59.1
Hello Production
```
...
```bash
$ echo "HELLO=REDACTED" > .env.REDACTED
$ echo "console.log('Hello ' + process.env.HELLO)" > index.js

$ dotenvx run -f=.env.REDACTED -- node index.js
Hello REDACTED
> ^^
```
...
Use [dotenvx](https://github.com/dotenvx/dotenvx) for REDACTED deploys.

Create a `.env.REDACTED` file.
...
```sh
$ dotenvx encrypt -f .env.REDACTED
```
...
Commit your `.env.REDACTED` file to code and deploy.

```
$ git add .env.REDACTED
$ git commit -m "encrypted .env.REDACTED"
...
We recommend creating one `.env` file per environment. Use `.env` for local/development, `.env.REDACTED` for REDACTED and so on. This still follows the twelve factor principles as each is attributed individually to its own environment. Avoid custom set ups that work in inheritance somehow (`.env.REDACTED` inherits values from `.env` for example). It is better to duplicate values if necessary across each `.env.environment` file.
