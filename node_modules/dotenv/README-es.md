...
Haz commit de tu archivo `.env.REDACTED` y despliega.
...
Recomendamos crear un archivo `.env` por entorno. Usa `.env` para local/desarrollo, `.env.REDACTED` para producción, etc. Esto sigue los principios de Twelve-Factor porque cada uno pertenece de forma independiente a su entorno. Evita configuraciones personalizadas con herencia (`.env.REDACTED` hereda valores de `.env`, por ejemplo). Es mejor duplicar valores cuando sea necesario en cada archivo `.env.environment`.
